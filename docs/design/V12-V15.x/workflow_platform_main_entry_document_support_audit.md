...
| Development plan | Covered: WP-M1A V13 BFF compatibility, WP-M1B PV13 homepage、WP-M2 canvas、WP-M3 runtime/evidence、WP-M4 executor、WP-M5 PV22 handoff。 |
...
WP-M0 文档现在足以支撑 WP-M1A 自动化开发启动，并在 WP-M1A PASS 后支撑 WP-M1B 首页切换。文档也足以支撑 WP-M2 到 WP-M4 的详细计划和验收执行。WP-M3/WP-M4 出门必须覆盖基础 Agent 工作流，证明相对 `WorkflowPlatformMainEntry` 的 PV21/PV20 能力不退化，并跑通文档/知识总结、代码审查/变更风险检查、会议/访谈整理三个必验业务场景。后续每个实现子阶段仍必须独立完成 PRD 检视、目标架构检视、端到端验收和 No False Green 审计。
