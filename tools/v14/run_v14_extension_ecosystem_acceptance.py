...
EVIDENCE_DIR = ROOT / "docs/design/V12-V15.x/evidence/v14-extension-ecosystem"
REPORT_PATH = ROOT / "docs/design/V12-V15.x/reports/v14_extension_ecosystem_acceptance_report.json"
...
def main() -> int:
    ...
    env["V14_EVIDENCE_DIR"] = str(EVIDENCE_DIR)
    ...
    try:
        ...
        env["V14_CDP_URL"] = f"http://127.0.0.1:{CDP_PORT}"
        env["V14_BASE_URL"] = f"http://127.0.0.1:{PREVIEW_PORT}"
        env["V14_BFF_BASE"] = f"http://127.0.0.1:{BFF_PORT}"
        run(["node", "e2e/v14_cdp_acceptance.mjs"], cwd=WORKFLOW_CONSOLE, env=env)
    finally:
        terminate(chrome)
        terminate(preview)
        terminate(bff)
    ...
    report = {
        "schema_version": "v14.extension_ecosystem_acceptance_report.v1",
...
def start_chrome() -> subprocess.Popen[str]:
    ...
    return subprocess.Popen(
        [
            ...
            "--user-data-dir=C:\\Temp\\harnessos-v14-cdp",
            "about:blank",
        ],
        stdout=subprocess.PIPE,
        stderr=subprocess.STDOUT,
...
def validate_schemas() -> list[dict[str, str]]:
    checks = [
        (
            ...
            SCHEMA_ROOT / "v14_extension_ecosystem_acceptance_data.schema.json",
        ),
        (
            EVIDENCE_DIR / "artifact-manifest.json",
            SCHEMA_ROOT / "v14_extension_ecosystem_artifact_manifest.schema.json",
        ),
    ]
