...
const cdpURL = process.env.PV19_CDP_URL || "http://127.0.0.1:9349";
const baseURL = process.env.PV19_BASE_URL || "http://127.0.0.1:4184";
const bffBaseURL = process.env.PV19_BFF_BASE || "http://127.0.0.1:18140";
...
try {
  ...
  const state = await readJson(`${bffBaseURL}/bff/pv19/workbench/state?app_id=reference_app&project_id=demo_a&workspace_id=local`);
  assert(state.schema_version === "pv19.runtime_workflow_platform.v1", "PV19 state schema is required");
  ...
  await page.waitForSelector('[data-testid="pv19-runtime-platform"]', { timeout: 20000 });
  await assertText(page, "pv19-platform-contract", "业务不定制平台核心");
  ...
  await assertText(page, "pv19-graph-validation", "valid");
  await page.getByRole("button", { name: "生成 Diff" }).click();
  await assertText(page, "pv19-graph-validation", "user_confirmed_required_before_publish");
  await page.getByTestId("pv19-graph-validation").screenshot({ path: resolve(evidenceDir, "02-graph-validate-and-diff.png") });

  await page.getByRole("button", { name: "发布版本" }).click();
  await assertText(page, "pv19-publish-run", "wfv_", 20000);
  await page.getByRole("button", { name: "运行工作流" }).click();
  await assertText(page, "pv19-run-status", "waiting_approval", 20000);
  ...
  await assertText(page, "pv19-human-action-after", "completed", 20000);
  await assertText(page, "pv19-run-status", "completed", 20000);
  ...
  await assertText(page, "pv19-evidence", allowedClaim, 20000);
...
function buildArtifactManifest() {
  ...
  return {
    ...
    status: "PASS",
    created_at: createdAt,
    artifacts: files.map((name) => ({ path: `docs/design/V12-V15.x/evidence/pv19-runtime-workflow-platform/${name}` })),
  };
}
...
<body>
  ...
  <p>本报告由 Chrome CDP 自动化执行真实用户路径生成。浏览器只访问 <code>/bff/pv19</code>，未直连 <code>/v1/rpc</code> 或内部 runtime 路径。</p>
  <section class="grid">
