...
test("WP-M1 to WP-M5A workflow platform main entry acceptance", async ({ page }) => {
  ...
  await expect(page.getByTestId("v13-editable-studio")).toBeVisible({ timeout: 20_000 });
  ...
  await page.locator('[data-port-side="out"]').first().hover();
  ...
  await expect(page.getByTestId("workflow-platform-capability-parity")).toContainText("No False Green pass", { timeout: 60_000 });
  ...
  await expect(page.getByTestId("workflow-platform-capability-parity")).toContainText("缺失证据 0", { timeout: 60_000 });
  ...
  await expect(page.getByTestId("workflow-platform-data-source-closure")).toContainText("normal_path_static_sources=0", { timeout: 20_000 });
  await expect(page.getByTestId("workflow-platform-quality-states")).toContainText("quality states=8", { timeout: 20_000 });
  await expect(page.getByTestId("workflow-platform-claim-matrix")).toContainText("PRD refs=20", { timeout: 20_000 });
  ...
  writeJson("dto-snapshot.json", {
    ...
    wp_m6_data_source_regions: wpM6DataSource.ui_regions.map((region: { region_id: string }) => region.region_id),
    wp_m9_artifacts: wpM9Artifacts.artifacts.map((artifact: { scenario_id: string }) => artifact.scenario_id),
    wp_m10_quality_states: wpM10Quality.states.map((state: { state_id: string }) => state.state_id),
    wp_m11_claim_refs: wpM11Claims.requirements.map((item: { requirement_id: string }) => item.requirement_id),
...
async function fetchDto(page: Page, path: string) {
...
function buildFrontendDataSourceClosureReport(dto: any) {
  return {
    ...
    ui_regions: dto.ui_regions.map((region: any) => ({
...
function buildGraphEditSaveReadbackReport(actionText: string) {
  const actions = [
...
function buildClaimToEvidenceMatrix(dto: any) {
  return {
    ...
    requirements: dto.requirements.map((item: any) => ({
