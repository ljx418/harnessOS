...
const cdpURL = process.env.PV18_CDP_URL || "http://127.0.0.1:9348";
const baseURL = process.env.PV18_BASE_URL || "http://127.0.0.1:4178";
const bffBaseURL = process.env.PV18_BFF_BASE || "http://127.0.0.1:18048";
...
try {
  ...
  await postJson(`${bffBaseURL}/__test/pv18/route-log/clear`, {});
  const state = await readJson(`${bffBaseURL}/bff/pv18/knowledge/state`);
  assert(state.schema_version === "pv18.knowledge_opc.v1", "PV18 state DTO schema is required");
  assert(state.connector_health?.execution_mode === "mcp_stdio", "PV18 must use real data_service mcp_stdio mode");
  assert(state.connector_health?.real_data_service === true, "PV18 real data_service flag is required");

  await page.setViewportSize({ width: 1500, height: 980 });
  await page.goto(`${baseURL}/?studio=pv18-knowledge-opc&app_id=reference_app&project_id=demo_a&workspace_id=local`, {
    waitUntil: "networkidle",
  });
  await page.waitForSelector('[data-testid="pv18-knowledge-opc"]', { timeout: 20000 });
  await assertText(page, "pv18-platform-generality", "通用平台");
  ...
  await assertText(page, "pv18-build-status", "completed", 90000);
  await page.getByRole("button", { name: "提问并生成引用" }).click();
  await assertText(page, "pv18-citation-status", "pass", 90000);
  ...
  await assertText(page, "pv18-correction-status", "pending_human_review", 20000);
  ...
  const source = await postJson(`${bffBaseURL}/bff/pv18/knowledge/sources/import`, {
    title: "HarnessOS PV18 E2E 复核资料",
    content: buildSourceContent(),
  });
  const build = await postJson(`${bffBaseURL}/bff/pv18/knowledge/builds/start`, { mode: "bounded_review" });
  const buildStatus = await readJson(`${bffBaseURL}/bff/pv18/knowledge/builds/${encodeURIComponent(build.build_id)}/status`);
  const query = await postJson(`${bffBaseURL}/bff/pv18/knowledge/query`, {
    query: "PV18 Knowledge OPC 如何证明业务工作流平台能力？",
  });
  const quality = await postJson(`${bffBaseURL}/bff/pv18/knowledge/quality-feedback`, {
    issues: [{ severity: "review", message: "citation review remains human gated" }],
    low_signal_sources: [],
  });
  const correction = await postJson(`${bffBaseURL}/bff/pv18/knowledge/correction-plan`, {});
  const evidence = await readJson(`${bffBaseURL}/bff/pv18/knowledge/evidence/summary`);
  const routeLog = await readJson(`${bffBaseURL}/__test/pv18/route-log`);
  ...
  assert(routeLog.forbidden_matches.length === 0, "PV18 route log must not contain forbidden matches");
  assert(query.citation_coverage?.status === "pass", "PV18 query must return citation coverage pass");
  assert((query.source_refs || []).length > 0, "PV18 query must return source refs");
  assert(correction.requires_human_review === true, "PV18 correction must require human review");
  assert(correction.auto_publish_allowed === false, "PV18 correction must block auto publish");
...
async function postJson(url, data, expectedStatus = 200) {
  ...
  const body = await response.text();
  assert(response.status === expectedStatus, `${url} expected ${expectedStatus}: ${body}`);
  return JSON.parse(body);
}
...
function buildDtoSnapshots(...dtos) {
  const routes = [
...
function buildAcceptanceData(state, source, build, query, quality, correction, evidence) {
  return {
    ...
    real_data_service: {
      status: "PASS",
      connector_id: state.connector_health.connector_id,
      execution_mode: state.connector_health.execution_mode,
    ...
    artifact_refs: [...(source.artifact_refs || []), ...(build.artifact_refs || []), ...(query.artifact_refs || [])],
...
function scenario(caseId, visible, evidenceRefs) {
  return { case_id: caseId, status: "PASS", user_visible_result: visible, evidence_refs: evidenceRefs, blocking_failure: false };
}
