...
const evidenceDir = resolve("../../docs/design/V12-V15.x/evidence/post-v15-product-runtime-hardening");
const createdAt = "2026-06-25T00:00:00Z";
const allowedClaim = "PV16 complete: product-runtime hardening pilot ready for review.";

test("PV16 product-runtime hardening evidence package", async ({ page, request, baseURL }) => {
  ...
  await request.post("/__test/pv16/route-log/clear");
  const health = await readJson(request, "/bff/pv16/system/health");
  expect(health.status).toBe("ok");
  expect(health.dependencies.v15).toBe("PASS");

  await page.setViewportSize({ width: 1500, height: 980 });
  await page.goto(`${baseURL}/?studio=pv16-product-runtime-hardening`, { waitUntil: "networkidle" });
  await expect(page.getByTestId("pv16-product-runtime-hardening")).toBeVisible();
  await expect(page.getByTestId("pv16-bff-source")).toContainText("BFF-backed");

  await page.getByTestId("pv16-run-entity-mutation").click();
  await expect(page.getByTestId("pv16-entity-crud")).toContainText("PASS");
  await page.getByTestId("pv16-run-ownership-denial").click();
  await expect(page.getByTestId("pv16-entity-crud")).toContainText("DENIED");

  await page.getByTestId("pv16-confirm-runtime-run").click();
  await expect(page.getByTestId("pv16-runtime-inspect")).toContainText("runtime_backed");
  await expect(page.getByTestId("pv16-runtime-inspect")).toContainText("true");

  await page.getByTestId("pv16-run-deployment-smoke").click();
  await expect(page.getByTestId("pv16-deployment-hardening")).toContainText("health: PASS");
  await expect(page.getByTestId("pv16-product-runtime-journey")).toContainText("产品实体");
  ...
  const entityMutation = await postJson(request, "/bff/pv16/entities/mutate", {
    scope: {
  ...
  const ownershipDenial = await postJson(request, "/bff/pv16/entities/ownership-denial", {}, 403);
  const runtimeRun = await postJson(request, "/bff/pv16/runtime/confirm-run", {});
  const runtimeInspect = await readJson(request, `/bff/pv16/runtime/inspect/${encodeURIComponent(runtimeRun.run_ref)}`);
  const deploymentProfile = await readJson(request, "/bff/pv16/deployment/profile");
  const deploymentSmoke = await postJson(request, "/bff/pv16/deployment/hardening-smoke", {});
  const journey = await readJson(request, "/bff/pv16/product-runtime/journey");
  const routeLog = await readJson(request, "/__test/pv16/route-log");
  ...
  expect(runtimeRun.runtime_backed).toBe(true);
  expect(runtimeRun.runtime_event_refs.length).toBeGreaterThan(0);
  expect(runtimeRun.trace_refs.length).toBeGreaterThan(0);
  expect(runtimeRun.artifact_refs.length).toBeGreaterThan(0);
  expect(runtimeRun.quality_refs.length).toBeGreaterThan(0);
  expect(deploymentSmoke.command_output.join("\n")).toContain("health: PASS");
